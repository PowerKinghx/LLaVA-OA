...
class FocalLoss(nn.Module):
    ...
    The focal loss automatically down-weights well-classified examples and focuses
    training on hard negatives. This is particularly useful for class imbalance.

    Args:
        alpha: class weights (optional), shape [num_classes]
    ...
    def forward(self, inputs, targets):
        ...
        p_t = torch.exp(-ce_loss)  # p_t = exp(-CE) = probability of correct class

        # Apply focal weight: (1 - p_t)^gamma
        # When p_t is high (easy samples), (1-p_t) approaches 0, loss is greatly reduced
        # When p_t is low (hard samples), (1-p_t) approaches 1, loss remains large
        focal_weight = (1 - p_t) ** self.gamma
        focal_loss = focal_weight * ce_loss

        # Optional: apply class weight alpha
...
class CLIPTrainer:
    def __init__(self, config):
        ...
        # Training set distribution: KL-0: 2286, KL-1: 1046, KL-2: 1516, KL-3: 757, KL-4: 173
        class_counts = torch.tensor([2286, 1046, 1516, 757, 173], dtype=torch.float32)
        total_samples = class_counts.sum()  # 5778

        # Calculate raw weights
        raw_weights = total_samples / (5 * class_counts)

        # Fix max/min ratio to 3.0 (gentler with gamma=2.0)
        # Linear mapping: map raw weight range to [1.0, 3.0]
        min_raw = raw_weights.min()
        max_raw = raw_weights.max()

        # Linear interpolation: weight_new = 1.0 + (weight - min) / (max - min) * (3.0 - 1.0)
        class_weights = 1.0 + (raw_weights - min_raw) / (max_raw - min_raw) * 2.0
        class_weights = class_weights.to(self.device)

        logger.info("=" * 60)
        logger.info("Using Focal Loss (gamma=2.0) with FIXED-RATIO class weights:")
        logger.info("(Max/Min ratio = 3.0 to balance with Focal weighting)")
        for i, (count, raw_w, smooth_w) in enumerate(zip(class_counts, raw_weights, class_weights)):
            logger.info(f"  KL-{i}: {int(count):4d} samples -> raw {raw_w:.3f} -> alpha {smooth_w:.3f}")
        logger.info(f"Weight ratio (KL-4/KL-0): {class_weights[4]/class_weights[0]:.2f}x")
        logger.info("=" * 60)

        # Use Focal Loss instead of standard cross-entropy
        # gamma=2.0: standard setting (strong hard sample mining)
        # alpha=[1.0 to 3.0]: fixed 3x ratio, avoid double amplification with Focal
...
class Config:
    def __init__(self):
        # ========================================================================
        # JMIR Revision: CLIP + Focal Loss Experiment
        # ========================================================================
        self.model_path = "your_clip_model_path"
        self.train_data_path = "your_train_data_path"
        self.val_data_path = "your_val_data_path"
